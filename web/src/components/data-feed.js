...
function DataFeed (props) {
  ...
  if (historicalPrices === null && ticker != 'ETF-DIRECTORY') {
    axios
      .get('https://api.passiv.com/api/v1/historical/' + ticker)
      .then((response) => {
  ...
  axios.get(`https://api.passiv.com/api/v1/ownedPercent/${props.slug.current.toUpperCase()}`)
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post('https://api.passiv.com/api/v1/emailsubscribe', {email: email})
      .then((response) => {
        setSuccess(true)
